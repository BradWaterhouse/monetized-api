...
const testWebhookKey = 'whsec_jDgfbycSTuQOYmcEwto0qUN8YsRjD88u';
...
app.post('/checkout', async (req, res) => {
    const session = await stripe.checkout.sessions.create({
        ...
        line_items: [
            {
                price: process.env.PRODUCT_PRICE_KEY,
            },
        ],
        ...
        success_url:
            'http://localhost:8888/checkout/success',
...
app.get('checkout/success', (req, res) => {
    res.send({ data: 'successfully registered to api' });
})


app.get('checkout/error', (req, res) => {
    res.send({ data: 'failed to register to api' });
})
...
app.post('/webhook', async (req, res) => {
    ...
    const webhookSecret = testWebhookKey;
